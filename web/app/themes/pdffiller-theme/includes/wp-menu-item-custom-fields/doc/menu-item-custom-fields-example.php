...
class Menu_Item_Custom_Fields_Example
{
    ...
    public static function init()
    {
        ...
        self::$fields = array(
            'columns-switch' => __('Show child items as columns: ', 'wpblog-theme'),
        );
    }
        ...
        if ($depth == 0) {
            ...
            foreach (self::$fields as $_key => $label) :
                ...
                <p class="description description-wide <?php echo esc_attr($class) ?>">
                    ...
                    $checked_attr = $value == "on" ? "checked" : "";

                    printf(
                        '<label for="%1$s">%2$s<input type="checkbox" id="%1$s" class="widefat %1$s" name="%3$s" ' . $checked_attr . ' /></label>',
                        esc_attr($id),
                        esc_html($label),
                        esc_attr($name)
                    ) ?>
