...
foreach ( $items as $item_id => $item ) :
	...
	if ( apply_filters( 'woocommerce_order_item_visible', true, $item ) ) {
		...
		// Quantity
		echo "\n" . sprintf( __( 'Quantity: %s', 'wpblog-theme' ), apply_filters( 'woocommerce_email_order_item_quantity', $item['qty'], $item ) );

		// Cost
		echo "\n" . sprintf( __( 'Cost: %s', 'wpblog-theme' ), $order->get_formatted_line_subtotal( $item ) );
		...
		if ( $show_download_links && $_product->exists() && $_product->is_downloadable() ) {
			...
			foreach ( $download_files as $download_id => $file ) {
				...
				if ( count( $download_files ) > 1 ) {
					$prefix = sprintf( __( 'Download %d', 'wpblog-theme' ), $i );
				} elseif ( $i == 1 ) {
					$prefix = __( 'Download', 'wpblog-theme' );
				}
